...
- 云下gpu类型idc节点仅支持NVIDIA系列显卡，包括：Volta（如 V100）、Turing（如 T4）、Ampere（如 A100、A10）。
...
### 2、NVIDIA-SMI驱动安装（仅限GPU节点）
1）NVIDIA驱动下载安装请参考：
- **NVIDIA Tesla驱动**：[安装 NVIDIA Tesla 驱动](https://cloud.tencent.com/document/product/560/8048)
- **NVIDIA GRID驱动**： [安装 NVIDIA GRID 驱动](https://cloud.tencent.com/document/product/560/30060)
2）安装CUDA请参考： [安装 CUDA](https://cloud.tencent.com/document/product/560/8064)
...
![](https://qcloudimg.tencent-cloud.cn/raw/01f2d2b94fd5f3e741ed16e0ae06efc1.jpg)
   -   **节点类型**：选择线下IDC节点资源类型，CPU节点和GPU节点在不同的节点池进行异构管理。
